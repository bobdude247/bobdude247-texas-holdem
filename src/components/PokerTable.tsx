...
import type { PublicHandPlayer, PublicMatchState } from '../domain/holdem/public'
import { formatChips } from '../presentation/format'
import { PlayingCard } from './PlayingCard'
import { Seat } from './Seat'

interface PokerTableProps { readonly match: PublicMatchState }

export function PokerTable({ match }: PokerTableProps) {
  ...
  return (
    <section aria-label="Texas Hold'em table" className="table-stage">
      <div className="table-felt">
        ...
        {players.map((player) => {
          const visibleCards = 'revealedHoleCards' in player ? (player as PublicHandPlayer).revealedHoleCards : undefined
          return <Seat isActing={hand?.actingSeat === player.seat} isBigBlind={hand?.bigBlindSeat === player.seat} isDealer={hand?.button === player.seat} isSmallBlind={hand?.smallBlindSeat === player.seat} key={player.id} player={player} visibleCards={visibleCards} />
        })}
      </div>
    </section>
  )
